...
import { RecipeSaveToastWrapper } from '@/components/recipe-save-toast-wrapper';
...
export default async function RecipePage({ params }: RecipePageProps) {
  try {
    // Properly handle and unwrap params with a consistent approach
    // Use await instead of use() to maintain consistency with generateMetadata
    const resolvedParams = params instanceof Promise ? await params : params;
    const id = resolvedParams.id;
    const recipe = await getRecipeById(id);
    ...
    return (
      <article className="container py-8">
        {/* Toast notification component wrapped in Suspense boundary */}
        <RecipeSaveToastWrapper />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {recipe.images.slice(0, 2).map((image, index) => (
                <div 
                  ...
                  style={{ position: 'relative' }} /* Explicitly set position for Next.js Image */
                >
                  <Image
                    src={`/images/${image}`}
    ...
    return (
      <div className="container py-8">
        <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-md">
          An error occurred while loading this recipe. Please try again.
        </div>
        <Link 
          href="/recipes" 
